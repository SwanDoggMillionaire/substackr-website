...
function ResearchPageInner() {
  ...
  useEffect(() => {
    if (initialWriter && initialNiche && initialMode === 'writer') {
      handleSearch(initialWriter, 'writer', initialNiche)
  ...
  const handleSearch = async (writerName: string, mode: ResearchMode, niche?: string) => {
    ...
    try {
      ...
      // Phase 2: Generate essay ideas (writer mode only, when niche provided)
      if (mode === 'writer' && niche && niche.trim().length >= 3) {
        setState('generating-ideas')

        const ideasRes = await fetch('/api/essay-ideas', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ profile: fetchedProfile, userNiche: niche.trim() }),
        })
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-gray-500 text-lg max-w-xl mx-auto">
            {state === 'success'
              ...
              : 'Enter a writer you admire and your niche. Get five essay ideas in seconds.'}
          </p>
        </div>
